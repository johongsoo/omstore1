...
# 웹 페이지 열기
url = 'https://m.shoppinghow.kakao.com/m/search/q/%EC%98%A4%EB%91%A5%EC%9D%B4%20%EC%9D%B8%ED%98%95'
driver.get(url)
...
if products:
    ...
    # 테이블 존재 여부 확인
    try:
        cursor.execute("SELECT * FROM odungstore WHERE ROWNUM = 1")
    except cx_Oracle.DatabaseError:
        print("테이블이 존재하지 않으므로 생성합니다.")
        create_table_query = '''
        CREATE TABLE odungstore (
    ...
    # 데이터 삽입을 위한 SQL 구문
    insert_query = '''
    INSERT INTO odungstore (productsid, name, category, price, image, link)
